    ...
    <div className="aa-page">
      ...
      <aside className={`aa-sidebar ${sidebarOpen ? 'aa-sidebar--open' : ''}`}>
        <div className="aa-sidebar__logo">
          ...
          <div className="aa-brand-text-wrap">
            ...
            <span className="aa-brand-sub">Secretary Console</span>
          </div>
        </div>
      ...
      <header className="aa-topbar">
        ...
        <div className="aa-topbar__right">
          <span className="aa-topbar__user-name">{profile?.full_name || 'Secretary'}</span>
          <div className="aa-topbar__avatar">{(profile?.full_name || 'ST').split(' ').map(p => p[0]).slice(0, 2).join('').toUpperCase()}</div>
        </div>
      </header>
