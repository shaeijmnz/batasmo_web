    ...
    <div className="al-page">
      ...
      <aside className={`al-sidebar ${sidebarOpen ? 'al-sidebar--open' : ''}`}>
        <div className="al-sidebar__logo">
          ...
          <div className="al-brand-text-wrap">
            ...
            <span className="al-brand-sub">Secretary Console</span>
          </div>
        </div>
      ...
      <header className="al-topbar">
        ...
        <div className="al-topbar__right">
          <span className="al-topbar__user-name">{profile?.full_name || 'Secretary'}</span>
          <div className="al-topbar__avatar">{(profile?.full_name || 'ST').split(' ').map((p) => p[0]).slice(0, 2).join('').toUpperCase()}</div>
        </div>
      </header>
