...
const extractOtp = (payload) => {
  const raw =
    payload?.email_otp ||
    payload?.properties?.email_otp ||
    payload?.user?.email_otp ||
    ''
...
export async function adminGenerateSignupOtp({ supabaseUrl, serviceKey, email, password }) {
  ...
  const payloads = []
  if (password) {
    payloads.push({ type: 'signup', email: normalizedEmail, password: String(password) })
  }
  payloads.push({ type: 'signup', email: normalizedEmail })
  payloads.push({ type: 'magiclink', email: normalizedEmail })
...
const resolveUnconfirmedUserId = async ({ supabaseUrl, serviceKey, email, userId }) => {
  ...
  if (!response.ok) {
    throw new Error('Unable to find this signup account.')
  }
...
const checkSendRateLimit = async ({ supabaseUrl, serviceKey, userId }) => {
  ...
  const sendCount = match ? Number(match[1]) : 0
...
/**
 * @returns {{ ok: true, userId: string, emailSent: boolean, emailError?: string }}
 */
export async function dispatchSignupEmailOtp({
  supabaseUrl,
  serviceKey,
  email,
  userId,
  ...
  if (!isSignupOtpEmailConfigured()) {
    throw new Error(
      'Verification email is not configured on the server. Ask admin to set Gmail SMTP on Render (same as welcome email).',
    )
  }
  ...
  if (!resolvedUserId) {
    throw new Error('Unable to send verification email for this account.')
  }
