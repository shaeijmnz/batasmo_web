    ...
    <div className="cr-page atty-page-wrapper">
      ...
      <header className="cr-topbar">
        ...
        <div className="cr-topbar__right" style={{ position: 'relative' }}>
          <button className="cr-icon-btn cr-notif-btn" onClick={() => setNotifOpen(v => !v)}>
            <BellIcon />
            <span className="cr-notif-badge">{notifications.filter(n => n.unread).length}</span>
          </button>
          {notifOpen && (
            <div className="cr-notif-panel">
              <div className="cr-notif-panel__header">
                <span>Notifications</span>
                <button className="cr-notif-panel__close" onClick={() => setNotifOpen(false)}>✕</button>
              </div>
              {notifications.map(n => (
                <div key={n.id} className={`cr-notif-item ${n.unread ? 'cr-notif-item--unread' : ''}`}>
                  <p className="cr-notif-item__text">{n.text}</p>
                  <span className="cr-notif-item__time">{n.time}</span>
                </div>
              ))}
            </div>
          )}
