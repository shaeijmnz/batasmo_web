...
const TotalIcon = () => (
  <span style={{ color: '#f5a623', fontSize: 22, fontWeight: 900, lineHeight: 1 }}>₱</span>
);
const MonthIcon = () => (
...
export default function AttorneyEarnings({ onNavigate, profile }) {
  ...
  const [stats, setStats] = useState({ totalNet: 0, monthNet: 0 });
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        setStats({ totalNet: data.totalNet, monthNet: data.monthNet });
        setLoadError('');
      } catch (error) {
        if (!isMounted) return;
        setTransactions([]);
